...
import { ParseNode, spacing, mathord, textord } from "./parseNode";
...
const lookupSymbol = function (
    ...
    fontName: string,
    mode: Mode,
): { value: string, metrics: null| CharacterMetrics } {
    let value = v;
    // Replace the value with its replaced value from symbol.js
    if (symbols[mode][value] && symbols[mode][value].replace) {
        value = symbols[mode][value].replace as string;
    }
    return {
        value: value,
...
type NODETYPE = "spacing" | "mathord" | "textord"
/**
 * Makes either a mathord or textord in the correct font and color.
 */
const makeOrd = function (
    group: spacing | mathord | textord,
        options: Options,
    ...
    if (text.charCodeAt(0) === 0xD835) {
        ...
        if (lookupSymbol(text, fontName, mode).metrics) {
            ...
            const parts : SymbolNode[] = [];
            for (let i = 0; i < text.length; i++) {
                const s = makeSymbol(text[i], fontName, mode, options, classes.concat(fontClasses));
                parts.push(s);
            }
...
const makeVList = function (params: VListParam, options: Options): DomSpan {
    ...
    const realChildren : HtmlDomNode[] = [];
    let minPos = depth;
    let maxPos = depth;
    let currPos = depth;
    for (let i = 0; i < children.length; i++) {
...
const fontMap: { [k:string]: { variant: FontVariant, fontName: string }} = {
    // styles
    "mathbf": {
        variant: "bold",
            fontName: "Main-Bold",
