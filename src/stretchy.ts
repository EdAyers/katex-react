...
import {LineNode, PathNode, SvgNode, Span} from "./domTree";
import buildCommon from "./buildCommon";
import mathMLTree, { MathNode } from "./mathMLTree";
import utils from "./utils";

import Options from "./Options";
import{ParseNode, AnyParseNode, accent, accentUnder, xArrow, horizBrace} from "./parseNode";
import{DomSpan, HtmlDomNode, SvgSpan} from "./domTree";

const stretchyCodePoint: {[k:string]: string} = {
    widehat: "^",
    widecheck: "ˇ",
    widetilde: "~",
    utilde: "~",
...
const katexImagesData: {
    [k:string]: ([string[], number, number] | [[string], number, number, string])
} = {
...
const svgSpan = function(
    group: accent | accentUnder | xArrow | horizBrace,
    options: Options,
): DomSpan | SvgSpan {
    ...
    function buildSvgSpan_(): {
        ...
        if (utils.contains(["widehat", "widecheck", "widetilde", "utilde"],
            ...
            // @ts-ignore
            const grp: accent | accentUnder = group;
