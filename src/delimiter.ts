...
type font =  "Size1-Regular" | "Size4-Regular";
...
const traverseSequence = function(
    ...
    for (let i = start; i < sequence.length; i++) {
        ...
        const metrics = getMetrics(delim, delimTypeToFont(item), "math");
        ...
        if (item.type === "small") {
            const newOptions = options.havingBaseStyle(item.style);
