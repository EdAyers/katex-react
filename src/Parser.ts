...
import {checkNodeType, infix, text, color, textord, supsub, colorToken, size, url} from "./parseNode";
import ParseError from "./ParseError";
import {combiningDiacriticalMarksEndRegex} from "./Lexer";
import Settings from "./Settings";
import SourceLocation from "./SourceLocation";
import {Token} from "./Token";
import{ParseNode, AnyParseNode, SymbolParseNode} from "./parseNode";
...
export default class Parser {
    ...
    handleInfixNodes(body: AnyParseNode[]): AnyParseNode[] {
        ...
        for (let i = 0; i < body.length; i++) {
            const node : infix | undefined = checkNodeType(body[i], "infix");
            if (node) {
                if (overIndex !== -1) {
                    throw new ParseError(
    ...
    handleUnsupportedCmd(): AnyParseNode {
        ...
        const textordArray : AnyParseNode[]= [];
     ...
     */
    parseAtom(breakOnTokenText?: BreakToken): null | AnyParseNode {
        // The body of an atom is an implicit group, so that things like
        // \left(x\right)^2 work correctly.
        const base = this.parseGroup("atom", false, undefined, breakOnTokenText);
        ...
        while (true) {
            ...
            if (lex.text === "\\limits" || lex.text === "\\nolimits") {
                ...
                const prime : textord = {type: "textord", mode: this.mode, text: "\\prime"};
     ...
     */
    parseColorGroup(optional: boolean): colorToken | null {
     ...
     */
    parseSizeGroup(optional: boolean): size | null {
        let res;
        let isBlank = false;
        if (!optional && this.nextToken.text !== "{") {
     ...
     */
    parseUrlGroup(optional: boolean): url | null {
        ...
        let result = /^\s*([^\\/#]*?)(?::|&#0*58|&#x0*3a)/i.exec(url);
        let protocol = (result !== null ? result[1] : "_relative");
