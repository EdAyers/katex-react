...
import { assertNodeType, checkNodeType, op, op_text } from "../parseNode";
...
export const htmlBuilder: HtmlBuilderSupSub<op> = (grp, options) => {
    ...
    if (group.symbol) {
        ...
        if (group.body) {
            ...
            const output: SymbolNode[] = [];
            group = group as op_text
...
const singleCharBigOps: { [k: string]: string } = {
    "\u220F": "\\prod",
    "\u2210": "\\coprod",
    "\u2211": "\\sum",
    "\u22c0": "\\bigwedge",
...
const singleCharIntegrals: { [k: string]: string } = {
    "\u222b": "\\int",
    "\u222c": "\\iint",
    "\u222d": "\\iiint",
    "\u222e": "\\oint",
